...
class AttentionSentRNN(nn.Module):
    def __init__(self, batch_size, sent_gru_hidden, word_gru_hidden, n_classes, bidirectional= True):        
        ...
        if bidirectional == True:
            ...
            self.final_linear = nn.Linear(2* sent_gru_hidden + 1, n_classes)
        else:
            ...
            self.final_linear = nn.Linear(sent_gru_hidden + 1, n_classes)
    ...
    def forward(self, word_attention_vectors, state_sent, age):
        ...
        # final classifier
        final_map = self.final_linear(torch.cat((sent_attn_vectors.squeeze(0), age.view(-1,1)), 1))
...
def train_data(mini_batch, age, targets, word_attn_model, sent_attn_model, word_optimizer, sent_optimizer, criterion):
    ...
    for i in range(max_sents):
        ...
        if(s is None):
            ...
            s = torch.cat((s,_s),0)            
    y_pred, state_sent, _ = sent_attn_model(s, state_sent, age)
...
def get_predictions(tokens, age, word_attn_model, sent_attn_model):        
...
def test_accuracy_mini_batch(token, age, labels, word_attn, sent_attn):
    y_pred = get_predictions(token, age, word_attn, sent_attn)
...
def test_data(mini_batch, age, targets, word_attn_model, sent_attn_model, criterion):    
    state_word = word_attn_model.init_hidden()
    state_sent = sent_attn_model.init_hidden()
    max_sents, batch_size, max_tokens = mini_batch.size()
    s = None
...
def gen_minibatch(tokens, labels, mini_batch_size, shuffle= False):
    for token, label in iterate_minibatches(tokens, labels, mini_batch_size, shuffle= shuffle):
        yield pad_batch(token[:,0]), Variable(torch.from_numpy(token[:,1].astype(np.float32)).cuda(), requires_grad= False), Variable(torch.from_numpy(label).cuda(), requires_grad= False)
...
def check_val_loss(val_tokens, val_labels, mini_batch_size, word_attn_model, sent_attn_model, criterion):
    ...
    for token, label in iterate_minibatches(val_tokens, val_labels, mini_batch_size, shuffle= True):
        val_loss.append(test_data(pad_batch(token[:,0]), Variable(torch.from_numpy(token[:,1].astype(np.float32)).cuda(), requires_grad= False), Variable(torch.from_numpy(label).cuda(), requires_grad= False), 
...
def train_early_stopping(fold, mini_batch_size, X_train, y_train, X_test, y_test, word_attn_model, sent_attn_model, 
    ...
    for i in range(1, num_epoch + 1):
        try:
            tokens, age, labels = next(g)
            loss = train_data(tokens, age, labels, word_attn_model, sent_attn_model, word_attn_optimiser, sent_attn_optimiser, loss_criterion)
            acc = test_accuracy_mini_batch(tokens, age, labels, word_attn_model, sent_attn_model)
            ...
            if i % print_val_loss_every == 0:
                ...
                if val_loss < min_val_loss:
                    ...
                    min_idx = i
                    torch.save(word_attn_model.state_dict(), 'saved_models/age/fold{}_word_attn.pth'.format(fold))
                    torch.save(sent_attn_model.state_dict(), 'saved_models/age/fold{}_sent_attn.pth'.format(fold))
...
def evaluate_hanage(batchsize):
    dbank = pd.read_json('dbank.json')
    X = dbank[['tokens','age']].sample(frac=1, random_state=20)
    ...
    while idx < 10:
        ...
        for i in range(len(X_train)):
            X_train[i][1] = (X_train[i][1]-mu)/std

        for i in range(len(X_validate)):
            X_validate[i][1] = (X_validate[i][1]-mu)/std

        for i in range(len(X_test)):
            X_test[i][1] = (X_test[i][1]-mu)/std
        ...
        trained_word_attn.load_state_dict(torch.load('saved_models/age/fold{}_word_attn.pth'.format(idx)))
        trained_sent_attn.load_state_dict(torch.load('saved_models/age/fold{}_sent_attn.pth'.format(idx)))
